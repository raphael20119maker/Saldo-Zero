...
const EditDebtModal: React.FC<EditDebtModalProps> = ({ isOpen, onClose, onUpdateDebt, debtToEdit }) => {
  ...
  useEffect(() => {
    if (debtToEdit) {
      ...
      setRemainingBalance(String(debtToEdit.remainingBalance));
      setMonthlyPayment(String(debtToEdit.monthlyPayment));
      setPaymentDay(String(debtToEdit.paymentDay));
      setInstallmentsPaid(String(debtToEdit.installmentsPaid));
      setInstallmentsTotal(String(debtToEdit.installmentsTotal));
    }
  }, [debtToEdit]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!debtToEdit || !name || !remainingBalance || !monthlyPayment || !paymentDay || !installmentsTotal) return;
  ...
  if (!isOpen || !debtToEdit) return null;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-xl w-full max-w-md m-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Form fields are the same as AddDebtModal, just pre-filled */}
          <div>
