...
const EditCreditCardModal: React.FC<EditCreditCardModalProps> = ({ isOpen, onClose, onUpdateCreditCard, cardToEdit }) => {
  ...
  useEffect(() => {
    if (cardToEdit) {
      ...
      setPaymentDay(String(cardToEdit.paymentDay));
      setFlag(cardToEdit.flag);
      setIssuer(cardToEdit.issuer || '');
      setType(cardToEdit.type || 'credit');
      setStatus(cardToEdit.status || 'active');
      setColor(cardToEdit.color || colors[0]);
      setInitialCommittedLimit(String(cardToEdit.initialCommittedLimit || ''));
      setCurrentInvoiceAmount(String(cardToEdit.currentInvoiceAmount || ''));
    }
  }, [cardToEdit]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!cardToEdit || !name || !limit || !closingDay || !paymentDay) return;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 overflow-y-auto">
      <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-xl w-full max-w-md m-4 my-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">Editar Cartão</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
           ...
           <div>
            <label>Status</label>
            <select value={status} onChange={e => setStatus(e.target.value as any)} className="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 shadow-sm">
          ...
          <div>
            ...
            <input type="text" value={issuer} onChange={e => setIssuer(e.target.value)} className="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 shadow-sm" />
          </div>
          ...
          <div className="flex justify-end gap-4 pt-4">
            ...
            <button type="submit" className="px-4 py-2 rounded-md text-white bg-primary hover:bg-primary/90 dark:bg-primary-dark dark:hover:bg-primary-dark/90">Salvar Alterações</button>
          </div>
        </form>
      </div>
    </div>
